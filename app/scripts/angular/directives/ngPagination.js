  ...
  return {
    ...
    link: function($scope, $element, $attrs) {
      ...
      var pagesWatcher = function(itemsPerPage, textFilter, predicate, reverse) {
        ...
        // First page items
        // Filter $scope.data directly to get the Angular way of binding, not attach to any other variables
        $scope.pageItems = function() {
        	return ($filter('orderBy')($filter('filter')($scope.data, $scope.textFilter), predicate, reverse)).slice(0, $scope.itemsPerPage);
        };
        
        $scope.results = $filter('orderBy')($filter('filter')($scope.data, $scope.textFilter), predicate, reverse);
