                ...
                compile: function() {
                  return {
                    ...
                    pre: function($scope, $element, $attr) {
                    	// This is for dynamic data source which will be changed
                    	// without re-loading page
                		  $scope.$watch('data', function dataWatcher(dataArray) {
                		    ...
                		    if (dataArray == null || dataArray == undefined || !angular.isArray(dataArray)) {
                		      $scope.data = [];
                		    }
                		  })
                    },
                    ...
                    post: function($scope, $element, $attr) {
                	    ...
                	    if (angular.isArray($scope.atPagination.propFilter)) {
                	      for (var i = 0; i < $scope.atPagination.propFilter.length; i++) {
                	        ...
                	        $scope.$watch('atPagination.filteringObj["' + $scope.atPagination.propFilter[i] + '"]', function searchContentWatcher(newValue) {
                	          return pagesWatcher($scope.itemsPerPage, $scope.atPagination.filteringObj, $scope.predicate);
                	        });
                	      }
                	    } else {
                	      $scope.propName = '$';
                	    ...
                	    var pagesWatcher = function(itemsPerPage, filteringObj, predicate, reverse) {
                	      ...
                	      $scope.atPagination.numberOfPages = ($scope.atPagination.results.length % $scope.itemsPerPage) == 0
                	        ? Math.floor($scope.atPagination.results.length / $scope.itemsPerPage)
                	    ...
                	    var itemsPerPageWatcher = function(newValue) {
                	      return pagesWatcher(newValue, $scope.atPagination.filteringObj, $scope.predicate);
                	    };
                	    ...
                	    $scope.atPagination.sort = function(predicate) {
                	      pagesWatcher($scope.itemsPerPage, $scope.atPagination.filteringObj, predicate);
                	    ...
                	    $scope.loadNextPage = function() {
                	      ...
                	      // pagination
                	        ...
                	        $scope.displayedNoP = $scope.numberOfPagesArrayForm.slice($scope.atPagination.startRange, $scope.atPagination.startRange + 10);
                	      }
                	    };
